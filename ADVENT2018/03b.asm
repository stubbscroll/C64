...
; algorithm: for each claim, loop over all other claims and check for
; intersection (by coordinate checking)
; runtime: 53 seconds
...
loop2	; done?
	...
	lda (zp2),y
	bpl +
	; we made it to the end of the inner loop. this means the claim in
	; the outer loop intersects with no other claims. this means we won
	; so output id here
	lda id+0
	sta inbcd+0
	lda id+1
...
+	; don't compare a claim with itself
	...
	; they don't intersect if one claim's minimum in some dimension is
	; greater (or equal than) than the other's maximum in the same dimension
	; calculate t1=right x for z1, t2=right x for z2
+	ldy #0
	lda (zp1),y
	ldy #4
	clc
	...
	sbc t1+1
	bcs loopinc
	; calculate t1=upper y for z1, t2=upper y for z2
	ldy #2
	lda (zp1),y
	ldy #5
	clc
	...
	bcc onext ; overlap
+	; no overlap!
loopinc	; next element in inner loop
	lda zp2+0
	clc
	adc #6
	sta zp2+0
	...
	inc zp2+1
+	jmp loop2
onext	; next element in outer loop
	lda zp1+0
	clc
	adc #6
	sta zp1+0
	bcc +
	inc zp1+1
	; assume the problem has a solution,
	; don't check if outer loop terminates
+	inc id+0
	bne +
	inc id+1
+	jmp loop1
