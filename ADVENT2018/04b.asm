...
; algorithm: first, sort the input (actually very difficult). then for
; each specific minute, loop over every sleep interval and for each guard,
; count how many times they sleep during this minute. take the max across all
; guards and all minutes
; runtime: 24 seconds
...
start	sei
	...
	inc guard
+	
	; go through input 60 times (once for each minute), find out how many
	; times each guard sleeps during that minute. then find the id of the
	; guard that slept the most during that minute and the minute
	lda #0
	sta minute
	sta best+0
	sta best+1
...
gloop2	ldy #4
	...
	cmp #2
	bne sanity
	; we have a sleep interval, check if our specific minute is here
	ldy #3
	lda minute
	cmp (zp1),y
	bcc gnext ; minute is before falling asleep, try next
	ldy #$b
	lda minute
	cmp (zp1),y
	bcs gnext ; minute is after waking up, try next
	; increase
	ldy #0
	lda (gptr1),y
	clc
...
gnext	jsr inczp1
	jsr inczp1
	jmp gloop2
gend	; all guards asleep on this minute counted, find max
	ldx #maxguard
	ldy #0
	sty gptr1+0
	sty gptr2+0
...
+	iny
	...
	cpx #60
	beq gdone
	jmp gloop0 ; start at top of input list
gdone	; adjust guard id
	lda bestid+1
	sec
	sbc guard
	sta bestid+1
	; output solution: bestm * bestid
	ldx bestid+0
	ldy bestid+1
	stx mul1z+0
	sty mul1z+1
